    ...
    <section className="py-24 sm:py-32 bg-surface-bg" id="features">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <header className="text-center mb-16">
          <p className="inline-block text-xs font-semibold tracking-wide uppercase text-brand-primary mb-2">
            Features
          </p>
          <h2 className="text-3xl sm:text-4xl font-bold text-text-heading max-w-2xl mx-auto tracking-tight font-heading">
            {content.headline}
          </h2>
        </header>
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="list">
          {content.items.map((item, i) => (
            <li 
              key={i} 
              className="flex flex-col gap-4 p-8 bg-surface-card border border-surface-border rounded-card transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-primary/40 hover:shadow-md"
            >
              <div 
